        ...
        for (int grooming = 1; grooming <= 5; grooming++) {
            ...
            while (moreData) {
                ...
                ResponseEntity<CatDto[]> responseEntity = restTemplate.exchange(
                        ...
                        CatDto[].class // 4. DogDto[].class: `ResponseEntity`가 반환될 때 응답 본문을 변환할 객체 타입을 지정합니다. 여기서는 `DogDto` 타입의 배열을 나타내는 `DogDto[].class`를 사용하여, JSON 응답이 `DogDto[]` 타입의 객체 배열로 변환되도록 지정합니다. `exchange` 메서드는 제네릭 타입으로 `ResponseEntity<T>`를 반환하므로, 이 타입 파라미터에 대응하는 클래스 객체를 넘겨줌으로써 응답 본문의 변환을 지시합니다.
                );
